...
parameters :: (Hashable move, StatefulGen g m) =>
	...
	-- | 'T.play'
	(position -> move -> m ()) ->
	g -> T.Parameters m Float Statistics move position
...
parametersIO :: Hashable move =>
	...
	(move -> Player) ->
	(position -> move -> IO ()) ->
	IO (RNG, T.Parameters IO Float Statistics move position)
...
parametersDeterministic :: (Hashable move, Monad m) =>
	...
	-- | 'T.play'
	(position -> move -> m ()) ->
	T.Parameters m Float Statistics move position
...
descend params@T.Parameters{} temperature g pos t = case weights of
	...
	where
	weight stats = realToFrac (visitCount stats ** temperature)
...
meanValuation :: Statistics -> Float
...
pucbA0 :: Float -> Player -> Statistics -> Statistics -> Float
...
data Moves move = Moves
	...
	, moveDistribution :: HashMap move Float
...
lost = finished . realToFrac . lostValuation

-- | A suitable summary of a game that's finished where neither player won.
drawn :: Moves move
drawn = finished (realToFrac drawnValuation)

fromMoves :: (Hashable move, StatefulGen g m) => Float -> Float -> g -> Moves move -> m (Statistics, HashMap move Statistics)
...
normalize :: HashMap a Float -> HashMap a Float
...
pucbA0Raw :: Float -> Float -> Float -> Float -> Float -> Float
...
-- 'parametersIO'.
dirichlet :: StatefulGen g m => Float -> (Float -> v -> v') -> g -> HashMap k v -> m (HashMap k v')
dirichlet param f g m = do
	d <- Dist.dirichlet (V.replicate (HM.size m) (realToFrac param)) g
	pure $ evalState (traverse (\v -> state (\i -> (f (realToFrac (d V.! i)) v, i+1))) m) 0
...
dirichletA0 :: StatefulGen g m => Float -> Float -> g -> HashMap move Float -> m (HashMap move Statistics)
